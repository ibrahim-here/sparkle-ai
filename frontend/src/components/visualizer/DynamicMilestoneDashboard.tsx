...
import { ChevronLeft, ChevronRight, Play, CheckCircle2, Lightbulb, Zap, Rocket, Info } from 'lucide-react';
import gsap from 'gsap';

const buttonStyles: any = {
    primary: "bg-primary text-white hover:shadow-primary/20",
    success: "bg-green-500 text-white hover:shadow-green-500/20",
    warning: "bg-yellow-500 text-white hover:shadow-yellow-500/20",
    danger: "bg-red-500 text-white hover:shadow-red-500/20"
};

export default function DynamicMilestoneDashboard({ milestones = [], onReset }: { milestones: any[], onReset: () => void }) {
    ...
    return (
        <div ref={containerRef} className="min-h-screen bg-secondary text-white font-sans p-4 md:p-8">
            <div className="max-w-7xl mx-auto">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    
                    {/* Visualizer focus (8 cols) */}
                    <div className="lg:col-span-8 space-y-4">
                        <div className="glass rounded-[1.5rem] p-4 lg:p-6 relative overflow-hidden group min-h-[460px] flex flex-col border border-white/5">
                           ...
                           <div className="flex items-center justify-between mb-4">
                               ...
                               <button 
                                    onClick={onReset}
                                    className="px-4 py-1.5 text-xs glass hover:bg-white/10 transition-all rounded-xl font-bold border-white/10"
                                >
                                    New Search
                                </button>
                           </div>
                           ...
                           <div className="mt-6 pt-4 border-t border-white/5 flex flex-wrap gap-3 justify-center">
                                {(() => {
                                    ...
                                    if (buttons.length === 0 && animations.length > 0) {
                                        animations.filter((a: any) => a.trigger).forEach((a: any) => {
                    ...
                    <div className="lg:col-span-4 space-y-4">
                        <div className="glass rounded-[1.5rem] p-6 h-full flex flex-col shadow-xl border border-white/5">
                            ...
                            <div className="flex-1 text-white/80 leading-relaxed overflow-y-auto max-h-[400px] pr-2 custom-scrollbar">
                                <div 
                            ...
                            <div className="mt-6 flex items-center justify-between pt-4 border-t border-white/5">
                                <button
                                    ...
                                    className="p-3 glass rounded-xl hover:bg-white/10 disabled:opacity-5 transition-all border-white/5"
                                >
                                    <ChevronLeft size={20} />
                                </button>
                                ...
                                <button
                                    ...
                                    onClick={() => setActiveIndex(prev => prev + 1)}
                                    className="p-3 bg-primary text-white rounded-xl hover:opacity-90 disabled:opacity-5 transition-all shadow-lg hover:shadow-primary/20"
                                >
